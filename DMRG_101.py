...
### 021: added tol option in dmrg engine because ARPPACK did not converge

### 100: new version
# updated H_eff with _matvec function to speed up immensly

### 101: 
# updated Ss update rule (not been used before). Now it is S[i] is right of M[i], so it is M[i] S[i]. Note that Ss[-1] = np.ones(1) dummy
# included loc_exp(self,loc_op,i) and all_exp(self,loc_op) in MPS class
...
import scipy.sparse # needed for H_eff object class
...
class random_MPS(object):
    ...
    def __init__(self,L,d,chi_max):
        ...
        self.Ss = [None]*L # initially meaningless but may be updated after normalization
        self.Ss[-1] = np.ones(1)
        self.L = L
        self.d = d
    ...
    def scalar_product(self,mps2):
        """calculate the norm of the MPS"""
        #C = 0
        #for j in range(self.d):
            #C += np.dot(self.Ms[0][j].transpose().conjugate(),self.Ms[0][j])
        
        C = np.tensordot(self.Ms[0].conjugate(),mps2.Ms[0],([0,1],[0,1])) # [1] {sl}  a'l, [1] {sl}  a'l
        for i in range(1,self.L):
            # sum over physical sites 1 .. L
            for j in range(self.d):
                # sum over physical index s_i = 1 .. d
                ...
                """
                temp1 = np.tensordot(C,mps2.Ms[i],[1,0]) #al [a'l], [al] sl  a'l
                C2 = np.tensordot(self.Ms[i].conj(),temp1,([0,1],[0,1])) # (T) [al] {sl} a'l , [al] {sl} a'l
            C = C2
        return C[0,0]
    ...
    def loc_exp(self,loc_op,i):
        """ Alternatively: Knowing that is in right-canonical form (all matrices are B matrices) after left-right sweep
        can use theta = S[i-1]B[i] = S[i-1]G[i]S[i]
        (This is because when using Theta the state is effectively left-right-canonical at the desired site)
        ----(Theta)----
        |      |      |
        |      |      |
        |    loc_op   |
        ...
        ----(Theta*)---
        """
        theta = np.tensordot(np.diag(self.Ss[i-1]),self.Ms[i],1) # a_l-1 s_l a_l
        dummy = np.tensordot(theta,theta.conj(),axes=([0,2],[0,2])) # (a_l-1) s_l (a_l) , # (a'_l-1) s'_l (a'_l)
        dummy = np.tensordot(dummy,loc_op,axes=([0,1],[0,1])) #  s_l s'_l , s_l s'_l
        return np.array([np.real_if_close(dummy)])[0]
    def all_exp(self,loc_op):
        dummy = []
        for i in range(self.L):
            theta = np.tensordot(np.diag(self.Ss[i-1]),self.Ms[i],1) # a_l-1 s_l a_l
            dumdum = np.tensordot(theta,theta.conj(),axes=([0,2],[0,2])) # (a_l-1) s_l (a_l) , # (a'_l-1) s'_l (a'_l)
            dumdum = np.tensordot(dumdum,loc_op,axes=([0,1],[0,1])) #  s_l s'_l , s_l s'_l
            dummy.append(dumdum)
        return np.real_if_close(dummy)
...
class DMRG(object):
    ...
    tol: accuracy in arppack arp.eigsh, std set to 0
    
    
    '''
    def __init__(self,MPS,MPO,eps=1e-10,chi_max=None,test=False,tol=0,maxiter=None):
        self.test = test
        ...
        # For testing: singular values
        #self.Ss = [None]*len(MPS.Ms)
    ...
    def site_update(self,i):
        ...
        e, v = arp.eigsh(heff, k=1, which='SA', return_eigenvectors=True, v0=vguess,tol=self.tol,maxiter=self.maxiter)
        self.es = e[0]
        if self.test:
    ...
    def next_LP(self,i):
        ...
        F = np.tensordot(F,self.MPS.Ms[i].conj(),axes=[2,0]) # a_i-1 b_i-1 [a'_i-1] , [a_i-1] s_i a'_i
        F = np.tensordot(self.MPO.Ws[i],F,axes=([2,0],[2,1])) # [b_i-1] b_i [s_i] s'_i , a_i-1 [b_i-1] [s_i] a'_i
        F = np.tensordot(self.MPS.Ms[i],F,axes=([0,1],[2,1])) # [a_i-1] [s_i] a_i , b_i [s'_i] [a_i-1] a'_i
    ...
    def next_RP(self,i):
        ...
        F = np.tensordot(self.MPS.Ms[i].conj(),F,axes=([2],[2])) # a'_i-1 s'_i [a'_i] , a_i b_i [a'_i]
        F = np.tensordot(self.MPO.Ws[i],F,axes=([3,1],[1,3])) # b_i-1 [b_i] s_i [s'_i] , a'_i-1 [s'_i] a_i [b_i]
        F = np.tensordot(self.MPS.Ms[i],F,axes=([1,2],[1,3])) # a_i-1 [s_i] [a_i] , b_i-1 [s_i] a'_i-1 [a_i]
...
class H_eff(scipy.sparse.linalg.LinearOperator):
    '''
    This is a rather abstract Linear Operator object for input into the Eigensolver
    It needs the scipy.sparse.linalg.LinearOperator.matvec(vector) to know how to multiply an input vector
    Here this input vector is the tensor M[a_l-1,s_l,a_l] in Vector-form and what matvec calculates is
    H_eff * M
    ...
    def __init__(self,LP,RP,W):
        ...
        self.LP = LP
        self.W = W
        chiL = LP.shape[0] # a_l-1
        chiR = RP.shape[0] # a_l
        d = W.shape[2] # s_l
        self.shape = (chiL*d*chiR,chiL*d*chiR)
        self.M_shape = (chiL,d,chiR)
        self.dtype = W.dtype # not sure whether this is important
    ...
    def _matvec(self,M):
        """ instructions on how to multiplay a vector M to H_eff, so to calculate H|Psi> effectively """
        temp = np.reshape(M,self.M_shape) # a'_l-1 s'_l a'_l
        temp = np.tensordot(self.RP,temp,axes=(2,2)) # R * M: a_l b_l (a'_l) , a'_l-1 s'_l (a'_l)
        temp = np.tensordot(self.W,temp,axes=([1,3],[1,3])) # W * temp: b_l-1 (b_l) s_l (s'_l) , a_l (b_l)  a'_l-1 (s'_l)
        temp = np.tensordot(self.LP,temp,axes=([1,2],[0,3])) # L * temp: a_l-1 (b_l-1) (a'_l-1) , (b_l-1) s_l a_l (a'_l-1)
        return np.reshape(temp,np.prod(temp.shape)) # a_l-1 s_l a_l
...
def expect_mpo(state,MPO):
    ...
    for i in range(state.L):
        F = np.tensordot(F,Ms[i].conj(),axes=[2,0]) # a_i-1 b_i-1 [a'_i-1] , [a_i-1] s_i a'_i
        F = np.tensordot(Ws[i],F,axes=([2,0],[2,1])) # [b_i-1] b_i [s_i] s'_i , a_i-1 [b_i-1] [s_i] a'_i
        F = np.tensordot(Ms[i],F,axes=([0,1],[2,1])) # [a_i-1] [s_i] a_i , b_i [s'_i] [a_i-1] a'_i
        # result is F with indices a_i b_i  a'_i
